...
const products: Product[] = [
  { img: heroShowroom, name: "Silk Midi Dress", price: "₽ 18 900", category: "Dresses" },
  { img: corporateImg, name: "Tailored Blazer", price: "₽ 24 500", category: "Outerwear" },
  { img: corporate2Img, name: "Cashmere Knit", price: "₽ 15 200", category: "Knitwear" },
  { img: kidsImg, name: "Linen Wrap Top", price: "₽ 12 800", category: "Tops" },
];

const ShowroomCollection = () => {
  const { lang, t } = useLanguage();
  const sc = t.showroomCollection;

  return (
    <section className="section-padding bg-secondary/25">
      <div className="container mx-auto">
        <ScrollReveal>
          <p className="overline text-primary mb-5 text-center">{sc.overline[lang]}</p>
          <h2 className="font-display text-4xl md:text-6xl font-light text-center mb-5 leading-[1.08]">
            {sc.title[lang]} <span className="italic">{sc.titleItalic[lang]}</span>
          </h2>
            ...
            <ScrollReveal key={product.name} delay={i * 100}>
              <Link to="/showroom" className="group block">
                ...
                <div className="text-center">
                  <h3 className="font-display text-base md:text-lg font-light mb-1.5 group-hover:text-primary transition-colors duration-300">{product.name}</h3>
                  <p className="text-sm text-muted-foreground font-light tracking-wide">{product.price}</p>
                </div>
              </Link>
            </ScrollReveal>
          ))}
