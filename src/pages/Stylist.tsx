...
const AIStylist = () => {
  ...
  <div className="mb-6 sm:mb-8">
      <p className="overline text-primary mb-3 font-medium">{label}</p>
      <div className="flex flex-wrap gap-2">
        {options.map((opt) =>
      <button
        key={opt}
        onClick={() => onChange(opt)}
        className={cn("px-4 sm:px-5 py-2 sm:py-2.5 text-[10px] sm:text-xs tracking-wider uppercase border transition-all duration-300 font-semibold",
